...
@Component(Command.METHOD_CALL_FWD_ESTABLISHED)
public class CmdCallFwdEstablished extends AbstractCommand {
	...
	public void exec(Command command, Boolean asServer) {
		...
		if (asServer) {
			callFwdDao.establishCallFwd(fwdEntity, invoker);
		} else {
			callFwdSecureDao.establishCallFwd(fwdEntity, invoker);
		}

		// Propagate command
		propagateCallFwdEstablished(invoker, fwdEntity);
	}
