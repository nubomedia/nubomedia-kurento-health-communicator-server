...
@Component(Command.METHOD_REMOVE_GROUP_ADMIN)
public class CmdRemoveGroupAdmin extends AbstractCommand {
	...
	public void exec(Command command, Boolean asServer) {
		...
		if (asServer) {
			...
			groupSecureDao.removeGroupAdmin(group, user);
		}

		propagateRemoveGroupAdmin(invoker, group, user);
