...
@Component(Command.METHOD_CALL_FWD_ACK)
public class CmdCallFwdAck extends AbstractCommand {
	...
	public void exec(Command command, Boolean asServer) {
		...
		if (asServer) {
			callFwdDao.ackCallFwd(fwdEntity, invoker);
		} else {
			callFwdSecureDao.ackCallFwd(fwdEntity, invoker);
		}

		// Propagate command
		propagateCallFwdAck(invoker, fwdEntity);
	}
