...
@Component(Command.METHOD_SEND_MESSAGE_TO_GROUP)
public class CmdSendMessageToGroup extends AbstractCommand {
	...
	public void exec(Command command, Content content, Boolean asServer) {
		...
		// Add this log to allow performance analysis
		khcLog.getLog("StartedMsgReceiving")
		...
		GroupEntity to = groupDao.findGroupByUUID(messageParam.getTo());
		...
		if (asServer) {
			...
			messageSecureDao.sendMessage(message, from, to, contentEntity);
		}
		log.debug("Message stored in database & assigned to destination timelines");
